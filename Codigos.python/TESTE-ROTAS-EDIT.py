...
coords = {
    ...
    'Rio Brilhante': (-21.8039, -54.5427)  # Nova cidade incluída
}
...
# Função para calcular a rota rodoviária entre dois pontos
def adicionar_rota(m, origem, destino, color="blue", weight=5, opacity=0.7):
    G = ox.graph_from_point(origem, dist=100000, network_type='all')  # Criar grafo ao redor da origem
    origem_node = ox.distance.nearest_nodes(G, origem[1], origem[0])
    destino_node = ox.distance.nearest_nodes(G, destino[1], destino[0])
    route = nx.shortest_path(G, origem_node, destino_node, weight='length')
    route_coords = [(G.nodes[node]['y'], G.nodes[node]['x']) for node in route]
    folium.PolyLine(route_coords, color=color, weight=weight, opacity=opacity).add_to(m)

# Adicionar a rota de Ponta Porã, Maracaju e Rio Brilhante para Santa Catarina
adicionar_rota(m, coords['Ponta Porã'], (-27.63813,-52.26715)) 
adicionar_rota(m, coords['Maracaju'], (-27.63813,-52.26715))  
adicionar_rota(m, coords['Rio Brilhante'], (-25.75090,-53.06035))  
adicionar_rota(m, coords['São Gabriel do Oeste'], (-27.09192,-52.62140))  
...
map_html = 'mapa_filiais_embarcadores_rotas.html'
m.save(map_html)
